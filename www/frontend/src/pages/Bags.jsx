...
function Bags() {
  ...
  console.log()
    ...
    ?.filter((d) => d.subCategory === "backpack")?.length 
  return (
    <>
     {
        loading ? <LoadingLogo /> :
      <div className="pt-[125px] pb-[40px]">
        <h2 className="text-center text-[35px] mb-10 font-light line-through">BACKPACKS / CARRY-ON</h2>
...
<div className="flex mx-10 my-4 gap-5">
        {products
        ?.filter((d) => d.subCategory === "carry_on") 
          ?.filter((item) => {
      ...
      {ProductCount === 0 && (
              <div className="flex w-full flex-col p-4 gap-1 justify-center items-center">
                <img src={assets.noluggage} alt="" className="w-32" />
                <p className="text-[20px] font-light text-gray-500 w-[300px] text-center">There are no Backpacks available at the moment! 😢 </p>
              </div>
            )}

        <div className="grid grid-cols-1 lg:grid-cols-4 place-items-center">
          {products
             ?.filter((d) => d.subCategory === "backpack") // Filter by subCategory
             .filter((d) => {
